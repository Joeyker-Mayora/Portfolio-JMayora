
import Agustin from '../../asset/Img/agustin.JPG'
import Eleazar from '../../asset/Img/Eleazar.jpeg'

export const DataTest = [
    {
        id: 1,
        image: Eleazar,
        name: "Eleazar Tracana",
        profession: "FullStack Software Developer | .Net | React | AWS",
        description: "Joeyker es una persona determinada con una alta capacidad resolutiva y un excelente nivel de ReactJS y Responsive Design. Es un profesional con sólidos valores humanos y habilidades para trabajar eficientemente en equipo."
    },
    {
        id: 2,
        image: Agustin,
        name: "Agustin Gimenez Pava",
        profession: "SRE Engineer | Python dev | Network engineer",
        description: "Tuve el placer de trabajar con Joeyker Mayora en el desarrollo de mi página web, y no puedo estar más satisfecho con los resultados. Joeyker es un desarrollador excepcional en ReactJS, con un profundo conocimiento y habilidad técnica que se reflejan en cada proyecto que toca."
    },
]